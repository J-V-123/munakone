...
int amount = 0;     // munien määrä
int doneness = 0;   // keittoaste
int cooktime;       // keittoaika
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {  // start timed event for read and send
    ...
    bookmarktime = millis()  +  60000 * cooktime;           // set the clock for 5 minutes in the future in milliseconds (1000 x 60 seconds x 6 minutes)
    state = 6;
    }
    else if (state == 6)  {                  
...
int menu2()  {    // keittoasteen valinta
  ...
  while(1){
    ...
    button2 = digitalRead(buttonPin2);
    button3 = digitalRead(buttonPin3);
    if (button1 == LOW)  {doneness = 1; return doneness;}
    else if (button2 == LOW)  {doneness = 2; return doneness;}
    else if (button3 == LOW)  {doneness = 3; return doneness;}
  }
}
